...
function proceedWithVerification(phoneNumber) {
    ...
    setTimeout(() => {
        ...
        // Mostrar código generado en consola para desarrollo
...
// Función para mostrar mensaje con código generado automáticamente
function showAutoGeneratedCodeMessage(code) {
    const codeModal = document.createElement('div');
    codeModal.className = 'auto-code-modal';
    codeModal.innerHTML = `
        <div class="auto-code-content">
            <div class="code-icon">
                <i class="fas fa-key"></i>
            </div>
            <h3>🔐 Código de Verificación</h3>
            <p>Tu código de verificación ha sido generado automáticamente:</p>
            <div class="generated-code">
                <span class="code-display">${code}</span>
                <button class="copy-code-btn" onclick="copyCodeToClipboard('${code}')">
                    <i class="fas fa-copy"></i>
                </button>
            </div>
            <div class="code-instructions">
                <p>• Ingresa este código en la siguiente pantalla</p>
                <p>• El código es válido por 10 minutos</p>
                <p>• Por seguridad, no compartas este código</p>
            </div>
            <button class="primary-btn" onclick="closeAutoCodeModal()">
...
// Función para cerrar modal de código automático
function closeAutoCodeModal() {
    const modal = document.querySelector('.auto-code-modal');
    if (modal) {
...
function sendFriendRequest(targetUserId, targetUserPhone) {
    ...
    database.ref(`friendRequests/${targetUserId}/${requestId}`).set(requestData)
        .then(() => {
            closeUserFoundCard();
            showSuccessMessage('✉️ Solicitud enviada correctamente');
...
function setupFriendRequestsListener() {
    ...
    friendRequestsListener.on('child_added', (snapshot) => {
        ...
        if (request && request.status === 'pending') {
            showFriendRequestModal(request, snapshot.key);
        }
    });
}
